...
- (void)main
{
	@try {
		...
		request = CFHTTPMessageCreateRequest(kCFAllocatorDefault, (CFStringRef)CFBridgingRetain([self requestMethod]), (CFURLRef)CFBridgingRetain([self url]), [self useHTTPVersionOne] ? kCFHTTPVersion1_0 : kCFHTTPVersion1_1);
		if (!request) {
			[self failWithError:ASIUnableToCreateRequestError];
			return;
		}
		...
		for (header in [self requestHeaders]) {
			CFHTTPMessageSetHeaderFieldValue(request, (CFStringRef)CFBridgingRetain(header), (CFStringRef)CFBridgingRetain([[self requestHeaders] objectForKey:header]));
		}
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
					...
					if ([credentials objectForKey:@"Authentication"]) {
						...
						if (CFHTTPMessageApplyCredentialDictionary(request, (CFHTTPAuthenticationRef)CFBridgingRetain([credentials objectForKey:@"Authentication"]), (CFDictionaryRef)CFBridgingRetain([credentials objectForKey:@"Credentials"]), NULL)) {
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		[self setReadStream:CFBridgingRelease(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)CFBridgingRetain([self postBodyReadStream])))];
    } else {
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {       
        ...
        if (clientCertificateIdentity) {
			...
			[certificates addObject:(id)CFBridgingRelease(clientCertificateIdentity)];
            ...
            CFReadStreamSetProperty((CFReadStreamRef)[self readStream], kCFStreamPropertySSLSettings, CFBridgingRetain(sslProperties));
        }
	...
	//
 	...
 	if ([self proxyHost] && [self proxyPort]) {
		...
		if ([[self proxyType] isEqualToString:(NSString *)kCFProxyTypeSOCKS]) {
			CFReadStreamSetProperty((CFReadStreamRef)[self readStream], kCFStreamPropertySOCKSProxy, CFBridgingRetain(proxyToUse));
		} else {
			CFReadStreamSetProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPProxy, CFBridgingRetain(proxyToUse));
		}
	}
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("ASIStreamID"), CFBridgingRetain([[self connectionInfo] objectForKey:@"id"]));
   ...
   // Start the HTTP connection
	CFStreamClientContext ctxt = {0, CFBridgingRetain(self), NULL, NULL, NULL};
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		if ([self numberOfTimesToRetryOnTimeout] > [self retryCount]) {
			...
			[self updatePartialDownloadSize];
			if ([self partialDownloadSize]) {
				CFHTTPMessageSetHeaderFieldValue(request, (CFStringRef)@"Range", (CFStringRef)CFBridgingRetain([NSString stringWithFormat:@"bytes=%llu-",[self partialDownloadSize]]));
			}
...
- (void)readResponseHeaders
{
	...
	[self setResponseHeaders: NSMakeCollectable(CFHTTPMessageCopyAllHeaderFields(message)) ];
	[self setResponseStatusCode:(int)CFHTTPMessageGetResponseStatusCode(message)];
	[self setResponseStatusMessage: NSMakeCollectable(CFHTTPMessageCopyResponseStatusLine(message)) ];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		NSString *httpVersion = NSMakeCollectable(CFHTTPMessageCopyVersion(message));
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		...
		[self setProxyAuthenticationScheme:NSMakeCollectable(CFHTTPAuthenticationCopyMethod(proxyAuthentication)) ];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(proxyAuthentication)) {
		[self setProxyAuthenticationRealm: NSMakeCollectable(CFHTTPAuthenticationCopyRealm(proxyAuthentication)) ];
	}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!requestAuthentication) {
		...
		CFRelease(responseHeader);
		[self setAuthenticationScheme: NSMakeCollectable(CFHTTPAuthenticationCopyMethod(requestAuthentication)) ];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		[self setAuthenticationRealm: NSMakeCollectable(CFHTTPAuthenticationCopyRealm(requestAuthentication)) ];
	}
...
{	
	...
	// Find out how much data we've uploaded so far
	[self setLastBytesSent:totalBytesSent];	
	[self setTotalBytesSent:[NSMakeCollectable(CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount))  unsignedLongLongValue]];
	[self setComplete:YES];
...
{
	NSError *underlyingError = NSMakeCollectable(CFReadStreamCopyError((CFReadStreamRef)[self readStream]));
...
#else
			NSDictionary *proxySettings = NSMakeCollectable(SCDynamicStoreCopyProxies(NULL));
#endif

			proxies = NSMakeCollectable(CFNetworkCopyProxiesForURL((CFURLRef)[self url], (CFDictionaryRef)proxySettings)) ;
...
- (void)runPACScript:(NSString *)script
{
	if (script) {
		...
		CFErrorRef err = NULL;
		NSArray *proxies = NSMakeCollectable(CFNetworkCopyProxiesForAutoConfigurationScript((CFStringRef)script,(CFURLRef)[self url], &err)) ;
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[JUMP_Reachability reachabilityForInternetConnection] startNotifier];
...
{
	return ([[JUMP_Reachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);	
}
